...
"""
Test the line_walk path finding module.
...
class TestPathFinder(tests.IrisTest):
    ...
    def test_line_walker(self):
        input_lats = [60, 70, 80]
        results = []
        for lat in input_lats:
            start, end = np.array((-180.0, lat)), np.array((180.0, lat))
            result = module.find_path(self.cube, line=(start, end))
